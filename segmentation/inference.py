...
def main():
    ...
    # loop dataset class
    folder_list = ['Normal', 'bacteria', 'TB', 'virus', 'COVID-19']
    for folder in folder_list:

        # Dataset
        print('\n>> Load dataset -', header.dir_data_root + folder)
        testset = mydataset.MyInferenceClass(tag = folder) # folder_list
        # testset = mydataset.MyInferenceCohen(tag = folder) # COVID-19
        # testset = mydataset.MyInferenceClassBIMCV()  # COVID-19
        # testset = mydataset.MyInferenceClassChest14()  # Normal
        # testset = mydataset.MyTestDataset(header.dir_test_path, test_sampler) # JSRT       
        ...
        with torch.no_grad(): 
            ...
            for i, data in enumerate(testloader, 0):
                ...
                for k in range(len(data['input'])): 
                    ...
                    # # png save
                    # Image.fromarray(post_output[1]*255 + post_output[2]*255).convert('L').save(save_dir + dir_case_id + '_mask.png')
                    # Image.fromarray(image_original.astype('uint8')).convert('L').save(save_dir + dir_case_id + '_image.png')
